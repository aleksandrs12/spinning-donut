...
def distance_to_color(d):
    ...
    output = [round(120 * (calculate_sin(the_procentage)+1)), round(120 * (calculate_cos(the_procentage)+1)), round(120 * (calculate_sin(the_procentage)+1))]
    if output[2] < 210:
        output[2] += 40
...
while run:
    ...
    for angle in np.arange(0, 2*pi, 2*pi / (360 / 1.5)):
        ...
        for angle2 in np.arange(0, pi*2, 2*pi / (360 / 1.5)):
            ...
            distance = ring_pos_adj[1] * 4 * x_rot_cos + x_rot_sin * angle1_cos * ((OUTER_RADIUS+INNER_RADIUS) / 2) + ((OUTER_RADIUS+INNER_RADIUS) / 2)
